...
public class Logic {
     ...
     */
    private volatile Map<Cell, Integer> neighborMap = new HashMap<>();
     ...
     * implementation.
     */
    private volatile Collection<Cell> cells = new HashSet<>(3000, 0.2f);
    ...
    public void tick() {
        ...
        Set<Touch> touches = context.getTouchData().getProcessed();
        for (Touch touch : touches) {
            cells.add(new Cell(touch.getX(), touch.getY()));
        }
    ...
    private void doLogic() {
        ...
        Collection<Cell> toRemove = new HashSet<>();
        // Cells that will be born during this generation
        Collection<Cell> toAdd = new HashSet<>();

        // New cell generation. Begins with a copy of the old one.
        Collection<Cell> newCells = new HashSet<>(cells);
        ...
        if (delta > TouchInputProcessTask.MIN_TICK_TIME) {
    ...
    private boolean isOutOfBounds(int x, int y) {
        ...
        if (x < -1 || y < -1
                || x > context.getGameView().getMatrixWidth() + 1
                || y > context.getGameView().getMatrixHeight() + 1) {
            return true;
        }
